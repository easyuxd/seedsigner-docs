# Set up a single-signature wallet
...
This is the recommended starting point for newcomers. A single-signature ("single-sig") wallet is the simplest self-custody setup: **one seed phrase controls all your funds.** It is easy to use, easy to back up, and easy to recover. By the end of this guide you will have:
...
> **Tip:** Do this entire walkthrough on **testnet first** — free practice coins, zero risk. See [Using testnet](/development/testnet.md). When you are confident, repeat it on mainnet with real funds.

---

## Single-sig vs. multi-sig — which should you choose?

| | Single-sig (this guide) | [Multi-sig](/security/multisig-walkthrough.md) |
|---|---|---|
...
- A SeedSigner device, [assembled](/hardware-build/assembly.md) and [running](/getting-started/first-boot.md).
- [Sparrow Wallet](https://sparrowwallet.com) installed on your computer (the "coordinator"). Other coordinators work too — see [Compatible wallets](/appendices/compatible-wallets.md).
...
The full instructions for every creation method are in [Create a new seed](/using-seedsigner/create-seed.md). If you already have a seed, [load it](/using-seedsigner/load-seed.md) instead and skip to Step 3.
...
3. Optionally, also create a [SeedQR backup](/using-seedsigner/seedqr-backup.md) so you can reload the seed in seconds later (you will use this in the recovery drill).
...
Leave this QR on screen for the next step. For the full reference, see [Export a public key (xpub)](/using-seedsigner/export-xpub.md).
...
Browse and double-check addresses any time with SeedSigner's [Address Explorer](/using-seedsigner/addresses.md) (**Tools → Address Explorer**). Use a **fresh receiving address for each payment** to protect your privacy.

---

## Step 7: Sign your first spend (optional now, essential later)

When you are ready to send Bitcoin, Sparrow builds an unsigned transaction (a **PSBT**), SeedSigner reviews and signs it, and Sparrow broadcasts it. The full, screenshot-by-screenshot guide is [Sign a transaction (PSBT)](/using-seedsigner/sign-transaction.md). In short:
...
1. **Wipe the seed from SeedSigner.** Either [discard the loaded seed](/using-seedsigner/discard-seed.md) (**Seeds → your seed → Discard**) or simply power the device off — being stateless, it forgets everything.
2. **Reload the seed from your backup**, exactly as a real recovery would go: type the words from paper, or scan your SeedQR. See [Load an existing seed](/using-seedsigner/load-seed.md).
...
If recovery succeeds, your single-sig wallet is fully operational and provably recoverable.
...
- **Sparrow can't read the xpub / SeedSigner can't read the QR.** Lower **QR Density** in SeedSigner settings for larger modules, improve lighting, and hold the screen 4–8 inches from the webcam. More tips: [Common issues — QR scanning](/troubleshooting/common-issues.md#qr-code-scanning-problems).
...
- [Sign a transaction (PSBT)](/using-seedsigner/sign-transaction.md) — the full spend flow.
- [Explore addresses](/using-seedsigner/addresses.md) — browse and verify any address.
- [Security model overview](/security/overview.md) — understand what SeedSigner does and does not protect.
- [Why multi-signature?](/security/why-multisig.md) — when to graduate from single-sig.
