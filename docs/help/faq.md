...
SeedSigner is fully open-source and community-audited. However, **you are your own software validation mechanism.** Always download releases from the official GitHub repository and verify the PGP signature and SHA256 hash before flashing. See [Download and Verify](/reference/software/verify-image.md).
...
A wallet descriptor defines how your multi-sig wallet is constructed: which public keys, in what order, with what rules. If you lose one seed AND the wallet descriptor, your funds are permanently lost. Always keep a backup of the descriptor. See [Wallet Descriptor](/reference/multisig/descriptor.md).
...
Sparrow Wallet, Specter Desktop, BlueWallet, Nunchuk, Keeper, and any wallet that supports PSBT exchange via QR codes. See [Compatible Wallets](/help/compatible-wallets.md).

### Does SeedSigner support testnet?

Yes. Change the network in Settings → Advanced → Bitcoin Network → Testnet. Pair it with Sparrow running in testnet mode for practice. See [Using Testnet](/contribute/testnet.md).
...
The QR code format is not recognized. Make sure you are scanning the correct type (PSBT, SeedQR, or wallet descriptor) and that the QR is clearly visible and well-lit. See [Error Messages](/help/error-messages.md).
