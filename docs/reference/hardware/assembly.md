...
- A flashed MicroSD card (see the [Software Setup guide](/reference/software/verify-image.md) for how to prepare this)
...
If you have a 3D-printed or commercial enclosure, install the assembled unit into it now. See the [enclosures page](/reference/hardware/enclosures.md) for available options.

No enclosure is strictly required — SeedSigner runs perfectly fine on an open bench while you get familiar with it.

## That's it

Plug in a Micro USB power source and your SeedSigner device will boot up. Head to [Power on and first boot](/reference/device/first-boot.md) for what to expect on screen. The first boot takes a little longer than usual, so give it a moment.
